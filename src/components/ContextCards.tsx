...
import { FileText, MessageCircle, Trash2, Upload } from 'lucide-react';
...
export const ContextCards: React.FC<ContextCardsProps> = ({
  ...
  const handleCreateGitHubIssue = useCallback(async () => {
    ...
    try {
      const conversationMessages: ChatContextMessage[] = cards.map((card) => ({
        id: String(card.id),
        content: `${card.title}\n${card.description || ''}\n${card.content || ''}`.trim(),
        isCode: false,
      ...
      const request: CreateIssueWithContextRequest = {
        ...
        description: 'This issue was generated from selected context cards.',
  ...
  return (
    <div className="h-full flex flex-col bg-bg terminal-noise">
      <div className="p-4 border-b border-border">
        ...
        <p className="text-xs text-muted font-mono mt-1">Curated chat and upload context for issue generation</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {cards.length === 0 ? (
          <div className="text-center py-12 animate-fade-in">
            ...
            <p className="text-muted text-xs font-mono">Add relevant chat or upload context for issue drafting</p>
          </div>
        ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    onClick={() => onRemoveCard(String(card.id))}
