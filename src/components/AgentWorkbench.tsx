...
export function AgentWorkbench(): JSX.Element {
  ...
  const [indexCodebase, setIndexCodebase] = useState(true);
    ...
    <div className="min-h-dvh bg-bg text-fg">
      ...
      <main className="mx-auto grid w-full max-w-[1800px] gap-4 px-4 py-4 sm:px-6 lg:grid-cols-[320px_minmax(0,1fr)_340px]">
        <RepositoryPanel
          ...
          onIndexCodebaseChange={setIndexCodebase}
  ...
  return (
    <aside className="grid auto-rows-max gap-4">
      ...
      <section className="rounded-lg bg-bg-secondary/72 p-3 shadow-[0_0_0_1px_rgba(255,255,255,0.08)]">
        ...
        <label className="mt-3 flex min-h-11 items-center justify-between gap-3 rounded-lg bg-bg-primary px-3 shadow-[0_0_0_1px_rgba(255,255,255,0.08)]">
          <span className="text-sm text-fg-secondary">Index codebase</span>
          <input
            checked={indexCodebase}
            className="size-5 accent-cyan"
            onChange={(event) => onIndexCodebaseChange(event.target.checked)}
            type="checkbox"
          />
        </label>
