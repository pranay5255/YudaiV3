...
Last updated: 2026-04-28 after adding review follow-up `#191`.
...
PR #188 remains the shipped backend step for Daifu tool calls: it added issue-publishing and fixed Architect, Tester, and Coder stage wrappers. Follow-up runtime work now lives in #175 and #179. A review follow-up bug tracker, #191, now captures workflow state/objective regressions that should be fixed before continuing session/workbench polish. The first low-hanging pass also completed the sandbox-provider decision (#167), Daifu issue/PR sizing guidance (#60), and the minimal PR complexity script (#63).

## Active Backlog

| Issue | Title | Area | Priority | Notes |
| --- | --- | --- | --- | --- |
| `#191` | Fix workflow state and execution objective regressions | Backend/session + Frontend | High | Review follow-up for partial context update clearing, stale selected issue URLs, and long issue bodies exceeding execution objective limits. See `docs/WORKFLOW_STATE_REVIEW_FOLLOWUP.md`. |
...
1. Fix review-found workflow regressions in `#191`.
2. Harden the current MSWEA runtime path in `#175`.
3. Align ChatOps and session handoff with that runtime in `#179`.
4. Polish frontend runtime/recovery UX in `#182` once backend events and failure states are stable.
5. Defer `#16` until PR metadata and execution artifacts are stable enough for a useful review table.
6. Decide the auth/org direction before touching `#139` or `#135`.
7. Leave `#143` until product ownership and shared execution identity are designed.
...
Current decision: keep Modal explicit and do not add a sandbox-provider
abstraction until a second provider requirement is concrete.
