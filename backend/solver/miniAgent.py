...
def main(
    ...
    github_issue: str | None = typer.Option(None, "--github-issue", help="URL to GitHub issue to solve as the task", show_default=False),
    ...
    # Use github_issue config if applicable and no custom config provided
    if github_issue and config_spec == DEFAULT_CONFIG:
        config_spec = "./config_mswea/mini.yaml"
    ...
    if github_issue:
        task = fetch_github_issue(github_issue)
        console.print("[bold green]Task set from GitHub issue.[/bold green]")
    elif not task:
    ...
    if github_issue:
        repo_url = github_issue.split("/issues/")[0]
        if github_token := os.getenv("GITHUB_TOKEN"):
