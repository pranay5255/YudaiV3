...
class FileContextItem(BaseModel):
    """File context item for issue creation"""
    id: str = Field(...)
    name: str = Field(...)
    type: str = Field(...)
    tokens: int = Field(..., ge=0)
...
class ChatContextMessage(BaseModel):
    """Chat message context for issue creation"""
    id: str = Field(...)
    content: str = Field(..., min_length=1)
...
class CreateIssueWithContextRequest(BaseModel):
    """Request model for creating issues with chat and file context"""
    title: str = Field(..., min_length=1, max_length=255)
    description: Optional[str] = Field(None)
...
class GitHubIssuePreview(BaseModel):
    """GitHub issue preview generated by architect agent"""
    title: str = Field(...)
    body: str = Field(...)
    labels: List[str] = Field(default_factory=list)
    assignees: List[str] = Field(default_factory=list)
    repository_info: Optional[Dict[str, str]] = Field(None)
