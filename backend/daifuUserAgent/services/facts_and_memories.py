...
class EmbeddingPipeline:
    ...
    def __init__(
        ...
        self._chunker = create_file_chunker(max_chunk_size=max_chunk_size, overlap=overlap)
...
class FactsAndMemoriesService:
    ...
    def _build_prompt(
        ...
        summary = snapshot.raw_response.get("raw_response", {}).get("summary", "No summary available.")
        tree = snapshot.raw_response.get("raw_response", {}).get("tree", "")
        files_preview = self._render_top_files(snapshot.files)
        convo_preview = self._render_conversation(conversation, max_messages=max_messages)
...
"""
    ...
    def _render_conversation(
        ...
        for message in recent:
            author = message.get("author") or message.get("sender") or message.get("role") or "user"
