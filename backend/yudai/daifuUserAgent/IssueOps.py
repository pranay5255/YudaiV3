...
from yudai.types import CreateUserIssueRequest
...
class IssueService:
    ...
    def _build_issue_generation_prompt(
        ...
        prompt_parts = [
            "You are an expert at creating Architect-ready GitHub issues from Daifu chat conversations.",
            "",
            "Goal: Produce a focused GitHub issue that the current Yudai Architect -> Tester -> Coder pipeline can execute reliably.",
            "The issue should give Architect enough repo-grounded context to refine scope, let Tester write focused tests,",
            "and let Coder implement and validate one PR without needing to infer product intent.",
            "",
            "INSTRUCTIONS (CRITICAL):",
            "1) Craft a precise title summarizing the task.",
            "2) Write a detailed body using the following sections in order:",
            "   - Summary (one paragraph of the user-visible problem or outcome)",
            "   - Repository evidence (specific files, routes, tests, issues, branches, commits, or probe findings from provided context)",
            "   - Scope (what this issue includes)",
            "   - Out of scope (what should wait or become a separate issue)",
            "   - Likely files (specific paths/packages/tests to inspect or change)",
            "   - Implementation plan (ordered, concrete steps for Architect/Coder to refine and execute)",
            "   - Acceptance criteria (checkbox list; binary verifiable)",
            "   - Tests (focused validation Tester should add or update)",
            "   - Risks / rollback (brief)",
            "   - Labels (suggested labels)",
            "3) Aim for one focused PR per issue. Prefer a single issue only when the likely change is around 150 LOC or smaller.",
            "4) Split or recommend separate issue drafts when work likely exceeds 200 LOC or spans unrelated subsystems.",
            "5) Include repo-native verification commands only when the provided context supports them; keep them non-interactive.",
            "6) If dependencies, file targets, or product behavior are ambiguous, state the uncertainty and ask for clarification instead of inventing details.",
            "",
        ...
        prompt_parts.extend(
            [
                "AGENT CONSTRAINTS:",
                "- The GitHub issue already exists before the fixed Architect -> Tester -> Coder execution starts.",
                "- Architect enriches issue/context; Tester writes tests/fixtures; Coder implements, validates, and opens the PR.",
                "- Do not describe deferred first-class agent framework work such as @task, SubagentExecutor, custom BaseTool stacks, or duplicate subagent classes.",
                "- Keep any commands non-interactive and repo-native when they are included.",
                "",
                "OUTPUT FORMAT:",
                "Return a JSON object with the following structure (extra keys allowed):",
                "{",
                '  "title": "Clear, descriptive issue title",',
                '  "body": "Detailed issue description with sections (Summary, Repository evidence, Scope, Out of scope, Likely files, Implementation plan, Acceptance criteria, Tests, Risks / rollback, Labels)",',
                '  "labels": ["label1", "label2"],',
                '  "assignees": []',
                "}",
                "",
                "Notes:",
                '- You may include additional keys like "mswea": {"handoff": {...}} if useful. The system will ignore unknown keys.',
                "- Do not require exact bash blocks when the repository context does not prove the commands.",
            ]
        )
