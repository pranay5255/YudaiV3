...
from yudai.models import ChatSession, FileEmbedding
    ...
    def get_embedding_model() -> SentenceTransformer:
        ...
        if LLMService._embedding_model is None:
            try:
                ...
                os.makedirs(model_config.hf_home, exist_ok=True)

                # Set environment variable for huggingface
                os.environ["HF_HOME"] = model_config.hf_home

                logger.info(f"Loading embedding model: {model_config.embedding_model}")
                logger.info(f"Using HF cache directory: {model_config.hf_home}")

                LLMService._embedding_model = SentenceTransformer(
                    model_config.embedding_model,
                    cache_folder=model_config.hf_home,
                ...
                try:
                    ...
                    LLMService._embedding_model = SentenceTransformer(
                        "all-MiniLM-L6-v2",
                        cache_folder=get_model_config().hf_home,
                        local_files_only=False
                    )
...
(Respond now as DAifu following the guidelines above. Keep responses conversational and under 200 words.)"""
    ...
    def read_github_context_cache(
        ...
        from yudai.context.cache_metadata import CacheMetadata
