...
from yudai.config.realtime_flags import RealtimeFeatureFlags, get_realtime_feature_flags
...
def should_run_modal_preflight(flags: Optional[RealtimeFeatureFlags] = None) -> bool:
    ...
    if not flags.modal_provisioning_enabled:
        return False
    return get_sandbox_config().modal_preflight_enabled
...
async def run_modal_preflight() -> ModalPreflightResult:
    ...
    sandbox = await RealtimeModalSandbox.create(
        ...
        timeout=sandbox_config.modal_preflight_sandbox_timeout_seconds,
    )

    try:
        healthcheck_url = await wait_for_sandbox_healthcheck(
            sandbox.tunnel_url,
            timeout_seconds=sandbox_config.modal_preflight_healthcheck_timeout_seconds,
        )
        exec_result = await run_modal_exec_smoke_test(
            tunnel_url=sandbox.tunnel_url,
            session_public_id=session_public_id,
            timeout_seconds=sandbox_config.modal_preflight_exec_timeout_seconds,
        )
