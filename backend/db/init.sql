...
-- Real-time session lifecycle tables (Phase 0 contract freeze)
...
SELECT create_trigger_if_not_exists(
    'update_sandboxes_updated_at',
    'sandboxes',
    'CREATE TRIGGER update_sandboxes_updated_at BEFORE UPDATE ON sandboxes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()'
);

SELECT create_trigger_if_not_exists(
    'update_session_runtime_updated_at',
    'session_runtime',
    'CREATE TRIGGER update_session_runtime_updated_at BEFORE UPDATE ON session_runtime FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()'
);
