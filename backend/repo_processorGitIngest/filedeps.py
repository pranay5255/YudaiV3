...
def save_file_items_to_db(
    db: Session, repository_id: int, file_items: List[FileItemResponse]
) -> List[FileItem]:
    ...
    def save_file_item_recursive(
        item: FileItemResponse, parent_id: Optional[int] = None
    ) -> FileItem:
...
def save_file_embeddings_to_db(
    ...
    current_usage = (
        db.query(func.sum(FileEmbedding.tokens))
        .filter(FileEmbedding.session_id == session_id)
        .scalar()
        or 0
    )
...
def process_file_content_for_embeddings(
    file_path: str, content: str, chunker: Any
) -> List[Dict[str, Any]]:
    ...
    for file_info in files_data.get("files", []):
        ...
        for i, part in enumerate(path_parts[:-1]):  # Skip the filename
            ...
            if current_path not in directories:
                ...
                dir_category = (
                    dir_category_info[0] if dir_category_info[0] else "Directory"
                )
...
def process_gitingest_data(raw_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "files": files,
        "repository_metadata": {"repository": repo_name, "url": repo_url},
    }
...
async def extract_file_dependencies(
    ...
    try:
        ...
        total_tokens = sum(
            estimate_tokens_for_file(f["path"], f["content_size"])
...
def search_similar_chunks(
    ...
    results = (
        db.query(
            FileEmbedding,
            (1 - FileEmbedding.embedding.cosine_distance(query_embedding)).label(
                "similarity"
            ),
        )
        .filter(
            FileEmbedding.session_id == session_id,
            (1 - FileEmbedding.embedding.cosine_distance(query_embedding))
            >= similarity_threshold,
        )
        .order_by((1 - FileEmbedding.embedding.cosine_distance(query_embedding)).desc())
        .limit(limit)
        .all()
    )
...
def get_session_token_usage(db: Session, session_id: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "avg_tokens_per_chunk": float(stats.avg_tokens_per_chunk)
        if stats.avg_tokens_per_chunk
        else 0.0,
    ...
    try:
        ...
        try:
            ...
            for file_info in repo_data.get("files", []):
                ...
                # Chunk the content
                chunks = process_file_content_for_embeddings(
                    file_path, placeholder_content, chunker
                )
                all_file_chunks.extend(chunks)

            # Save file embeddings to database
            if all_file_chunks:
                save_file_embeddings_to_db(
                    db, session.id, repository.id, all_file_chunks
                )
